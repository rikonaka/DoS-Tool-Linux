static int GetHTTPResponseData(char **output_buf, const char *input_buf, int debug_level)
{
    /*
    ...
    char *ptmp;
    char *response;
    ptmp = (char *)strstr(input_buf, "HTTP/1.1");
...
int Attack_GuessUsernamePassword(const pInput process_result)
{
    ...
    DisplayInfo("Load guess attack module...");
    ...
    DisplayDebug(DEBUG_LEVEL_2, process_result->debug_level, "username_type: %d\n", attack_struct->username_type);
    ...
    else
    {
        ...
        if (!fp)
        {
            DisplayError("Can not open the password file");
            return -1;
        }
        /* use the linked list */
    ...
    DisplayDebug(DEBUG_LEVEL_2, process_result->debug_level, "password_type: %d\n", attack_struct->password_type);
    ...
    DisplayDebug(DEBUG_LEVEL_2, process_result->debug_level, "Return value: %s", rebuf);

    // Guess the password
    if (GetHTTPResponseData(rebuf, result_buf, process_result->debug_level) != 0)
    {
    ...
    if (SuccessOrNot(process_result->debug_level, rebuf) == 0)
    {
        ...
        DisplayDebug(DEBUG_LEVEL_2, process_result->debug_level, "[%s : %s]", attack_struct->username, attack_struct->password);
        return -1;
    }
